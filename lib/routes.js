				    ...
				    if (res.statusCode == 200) {
						fs.writeFile(filepath,query,'utf8', function (err) {
							if (err) {
								...
								if (config.get('app.publicPort') != 80 && config.get('app.publicPort') != "") {
									publicPort = ":" + config.get('app.publicPort');
								} else {
									publicPort = "";
								}
								queryUrl = config.get('app.publicScheme') + "://" + config.get('app.publicHostname')
